export const personalInfo = {
  name: 'Prakash Balasubramanian',
  title: 'Senior Software Engineer | Technology Lead',
  subtitle: 'Frontend Specialist | Angular | React | 12+ Years Experience',
  email: 'prakash.engg89@gmail.com',
  phone: '+91 9944363757',
  location: 'Chennai, India',
  linkedin: 'https://www.linkedin.com/in/prakash-b-941885341/',
  github: 'https://github.com/Prakash-Balu',
  summary:
    'Experienced Senior Software Engineer with 12+ years in enterprise web development, skilled at steering technology initiatives, fostering teamwork, and driving professional growth. Seeking a Technology Lead position. Proficient in Angular, HTML, CSS, JavaScript, and Bootstrap. Collaborative and proactive team player focused on shared goals and continuous growth.',
};

export const skills = {
  primary: ['AngularJS', 'Angular 8', 'Angular 11', 'Angular 14', 'Angular 19', 'HTML5', 'CSS3', 'JavaScript', 'Bootstrap'],
  secondary: ['React', 'Node.js', 'jQuery', 'AJAX', 'TypeScript'],
  database: ['MySQL'],
  tools: ['JIRA', 'Git', 'SVN', 'Mantis', 'RQM'],
  os: ['Windows', 'Linux'],
  aiTools: ['Microsoft Copilot', 'GitHub Copilot', 'Claude AI'],
};

export const experience = [
  {
    role: 'Senior Software Engineer B2 (Technology Lead)',
    company: 'Cognizant Technology Solutions',
    location: 'Chennai, India',
    period: 'May 2025 – Jan 2026',
    project: 'Telstra Health',
    description:
      'Worked on Telstra Health — a major initiative to enhance the healthcare system in Australia. Redesigned the data architecture of My Health Record using the global FHIR standard for improved data interoperability. Connected care providers across hospital, health service, general practice, pharmacy, and aged care sectors.',
    responsibilities: [
      'Analysed user stories, functional specs and acceptance criteria for Clinical Manager modules (Care Statement, Allied Health, Medications, eNRMC)',
      'Designed, developed and enhanced application features ensuring adherence to clinical workflows and regulatory standards',
      'Delivered UI/UX consistency improvements across modules for seamless user experience',
      'Conducted detailed root cause analysis for complex defects across application layers',
      'Collaborated with Product Owners, QA, Support, Solution Architects, and DevOps for smooth SDLC delivery',
      'Created and maintained technical documentation (LLD) for developed modules',
      'Managed delivery across multiple parallel workstreams ensuring timelines and quality benchmarks',
    ],
    tech: ['AngularJS', 'Angular 14', 'HTML', 'CSS', 'JavaScript', 'Bootstrap'],
  },
  {
    role: 'Technology Lead',
    company: 'Infosys Limited',
    location: 'Chennai, India',
    period: 'Apr 2022 – Apr 2025',
    project: null,
    description:
      'Led frontend development teams and technology initiatives for enterprise web applications.',
    responsibilities: [
      'Led frontend development teams for enterprise-grade Angular applications',
      'Drove architectural decisions and code quality standards',
      'Mentored junior developers and conducted code reviews',
      'Coordinated with cross-functional teams to deliver projects on time',
    ],
    tech: ['Angular', 'HTML5', 'CSS3', 'JavaScript', 'Node.js'],
  },
  {
    role: 'Technology Analyst',
    company: 'Infosys Limited',
    location: 'Chennai, India',
    period: 'Aug 2018 – Mar 2022',
    project: null,
    description:
      'Developed and maintained enterprise web applications using Angular and modern frontend technologies.',
    responsibilities: [
      'Developed scalable Angular components and modules',
      'Performed performance optimisations to improve system responsiveness',
      'Implemented RESTful API integrations',
      'Collaborated with UI/UX designers to implement pixel-perfect interfaces',
    ],
    tech: ['Angular', 'HTML5', 'CSS3', 'JavaScript', 'Bootstrap'],
  },
  {
    role: 'Senior Systems Engineer',
    company: 'Infosys Limited',
    location: 'Chennai, India',
    period: 'Dec 2016 – Jul 2018',
    project: null,
    description:
      'Contributed to web development projects using AngularJS and related technologies.',
    responsibilities: [
      'Built AngularJS single-page applications',
      'Participated in Agile ceremonies including sprint planning and retrospectives',
      'Wrote unit tests and ensured code coverage',
    ],
    tech: ['AngularJS', 'HTML5', 'CSS3', 'JavaScript'],
  },
  {
    role: 'Software Engineer',
    company: 'Knobay Technologies Pvt. Ltd.',
    location: 'Chennai, India',
    period: 'Apr 2014 – Dec 2016',
    project: null,
    description:
      'Started career as a software engineer developing web applications for various clients.',
    responsibilities: [
      'Developed web applications using HTML, CSS, and JavaScript',
      'Built dynamic UI components and handled client-side logic',
      'Collaborated with backend teams for API integration',
    ],
    tech: ['HTML', 'CSS', 'JavaScript', 'jQuery'],
  },
];

export const projects = [
  {
    name: 'Telstra Health — Clinical Manager',
    description:
      'Healthcare platform for Australian care providers. Redesigned the data architecture of My Health Record using the FHIR standard for improved data interoperability across hospitals, pharmacies, and aged care sectors.',
    tech: ['Angular 14', 'AngularJS', 'HTML5', 'CSS3', 'JavaScript', 'Bootstrap'],
    type: 'Enterprise',
    highlight: true,
  },
  {
    name: 'Global Safety & Security Consultant',
    description:
      'A professional web application for a global safety and security consulting firm, providing information about services, expertise, and case studies.',
    tech: ['HTML5', 'CSS3', 'JavaScript', 'Angular'],
    type: 'Web App',
    highlight: false,
  },
  {
    name: 'Naduvai Address Book',
    description:
      'A community address book web application enabling residents to find and connect with local contacts easily.',
    tech: ['HTML5', 'CSS3', 'JavaScript'],
    type: 'Web App',
    highlight: false,
  },
  {
    name: 'Aalam Vizhuthugal',
    description:
      'A cultural community web platform designed to promote and preserve local heritage events and activities.',
    tech: ['HTML5', 'CSS3', 'JavaScript'],
    type: 'Website',
    highlight: false,
  },
  {
    name: 'Maaruthi Ladies Tailoring',
    description:
      'A business website for a tailoring service, showcasing services, gallery, and contact information.',
    tech: ['HTML5', 'CSS3', 'JavaScript', 'Bootstrap'],
    type: 'Website',
    highlight: false,
  },
  {
    name: 'Ayd Technologies',
    description:
      'Corporate website for a technology company, featuring service offerings, team profiles, and client testimonials.',
    tech: ['HTML5', 'CSS3', 'JavaScript', 'Bootstrap'],
    type: 'Website',
    highlight: false,
  },
];

export const education = [
  {
    degree: "Bachelor of Engineering (Information Technology)",
    institution: 'Annamalai University, Chidambaram',
    period: '2006 – 2010',
    grade: null,
  },
  {
    degree: 'Higher Secondary School (HSC)',
    institution: 'Sri Ramakrishna Vidyasala Higher Secondary School, Chidambaram',
    period: '2004 – 2006',
    grade: null,
  },
  {
    degree: 'Secondary School (SSLC)',
    institution: 'Sri Ramakrishna Vidyasala Higher Secondary School, Chidambaram',
    period: '2003 – 2004',
    grade: null,
  },
];

export const competencies = [
  {
    title: 'Senior Frontend Developer',
    description: 'Proficient in designing and delivering scalable enterprise web applications in Angular',
    icon: '💻',
  },
  {
    title: 'Team Leadership',
    description: 'Proven ability to lead development teams, mentor engineers, and drive technical excellence',
    icon: '👥',
  },
  {
    title: 'Communication',
    description: 'Excellent verbal and written communication with a talent for building strong stakeholder relationships',
    icon: '🗣️',
  },
  {
    title: 'Agile Methodology',
    description: 'Experienced in Agile/Scrum practices — sprint planning, reviews, and continuous delivery',
    icon: '⚡',
  },
];
